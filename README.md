...
- **Synchronous Tooling**: Automatically polls RSTUF tasks until completion.
- **Complete API Coverage**: Tools for bootstrapping, artifact management, 
  metadata rotation, and delegation.
- **Type Safe**: Built with TypeScript for reliable tool definitions.
...
| Tool | Description |
|------|-------------|
| `get_system_status` | Checks the bootstrap status of the TUF repository. |
| `initialize_repository` | Bootstraps the system with signed metadata. |
| `add_artifacts` | Adds artifacts to the metadata. |
| `publish_artifacts` | Publishes pending artifacts. |
| `rotate_metadata` | Rotates role metadata (requires offline signing). |
| `get_pending_signatures` | Lists roles pending signatures. |
| `add_signature` | Adds a signature for a metadata role. |
| `update_config` | Updates configuration settings. |
| `manage_delegation` | Creates, updates, or deletes TUF delegations. |
